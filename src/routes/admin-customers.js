...
} = require("../lib/driveUpload");
const {
  getPayments,
  registerPayment,
  deletePayment,
} = require("../lib/db-payments.js");
const MAX_SIZE = 1e7;
...
router.post("/add", upload.array("files", 128), async (req, res) => {
  ...
  try {
    ...
    const newCustomer = {
      ...
      opportune_payment_date: opportune_payment_date || null,
      max_payment_date: max_payment_date || null,
      type: "customer",
    };
    const [{ insertId }] = await pool.query("INSERT INTO customers set ?", [
      newCustomer,
    ]);

    createFolder(document, req.user.folderId).then(async (folderId) => {
      await pool.query("UPDATE customers set folderId = ? where id = ?", [
        folderId,
        insertId,
      ]);
    ...
    req.flash(
      "success",
      "Usuario registrado correctamente, los archivos estarán listos en unos segundos"
    );
    ...
    if (err.code == "ER_DUP_ENTRY") {
      req.flash(
        "message",
        `El documento ${document} ya está registrado en el sistema`
      );
      res.redirect("/admin/customers/add");
    } else {
...
router.get("/", isLoggedIn, async (req, res) => {
  ...
  try {
    const rows = await pool.query("SELECT * FROM customers WHERE user_id = ?", [
      req.user.id,
    ]);
    const customers = rows[0].reverse();
    for (let customer of customers) {
      customer.photoUrl = customer.photoId
        ? `https://drive.google.com/thumbnail?id=${customer.photoId}`
        : "https://www.freeiconspng.com/uploads/user-icon-png-person-user-profile-icon-20.png";
    }
...
router.delete("/delete/:id", async (req, res) => {
  ...
  try {
    ...
    deleteFolderAndContents(fileId).then(async () => {
...
router.get("/edit/:id", isLoggedIn, async (req, res) => {
  try {
    const { id } = req.params;
    const customers = await pool.query("SELECT * FROM customers WHERE id = ?", [
      id,
    ]);
...
router.post(
  ...
  async (req, res) => {
    ...
    const fileId = await uploadFile(
      picture,
      "1wMq4IRQBC-TA0pFdX-6_6U5kZLkjH6vr"
    );
    try {
      pool
        .query("UPDATE customers SET photoId = ? WHERE ID = ?", [fileId, id])
        .then(async () => {
          if (photoId != -1) {
...
router.post(
  "/edit/:userId/:folderId",
  upload.array("files", 128),
  async (req, res) => {
    ...
    try {
      ...
      uploadMultipleFiles(files, folderId).then(async () => {
        await renameFolder(folderId, document);
        console.log("Folder updated successfully");
      });
      req.flash(
        "success",
        "Cliente editado con éxito. Los archivos estarán listos en unos segundos"
      );
      res.redirect("/admin/customers");
    } catch (err) {
      console.log(err);
...
router.get("/payments/:userId", isLoggedIn, async (req, res) => {
  ...
  res.render("customers/customer-payments", {
    payments,
    userId,
    allowDelete: true,
  });
...
router.post("/payments/add/:userId", async (req, res) => {
  ...
  const { paid_amount, pending_amount, description, recipient, address } =
    req.body;
  const payment = {
